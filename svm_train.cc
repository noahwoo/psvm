...
void SvmTrainer::TrainModel(const Document& doc, const Kernel& kernel,
  ...
  if(mpi->GetProcId() == 0 ) {
	  printf("Solve QP with algorithm %d. \n", parameter.algorithm);
  }
  if( parameter.algorithm == 0 ) {
	  ipm_solver.Solve(parameter, icf_result, doc, model, failsafe);
  } else if( parameter.algorithm == 1) {
	  ipm_solver.SolveByMPC(parameter, icf_result, doc, model, failsafe);
  } else {
	  if(mpi->GetProcId() == 0 ) {
		  printf("No supported algorithm: %d, only (0:IPM, 1:MPC) supported \n", parameter.algorithm);
	  }
	  Usage();
	  exit(-1);
  }
...
double FLAGS_gamma_f   = 0.99;
double FLAGS_gamma_a   = 1.0/(1.0 - FLAGS_gamma_f);
double FLAGS_tau       = 3;
int FLAGS_algorithm    = 0;
...
void Usage() {
  char* msg =
      ...
      "      default: 1.0000000000000001e-09\n"
  	  "    -algorithm(Algorithm to use for solving) type: int(0: IPM, 1: MPC)\n"
      "      default: 0\n";
  cerr << msg;
}
...
void ParseCommandLine(int* argc, char*** argv) {
  ...
  for (i = 1; i < *argc; ++i) {
    ...
    } else if (strcmp(param_name, "algorithm") == 0) {
        FLAGS_algorithm = atoi(param_value);
...
int main(int argc, char** argv) {
  ...
  ipm_parameter.gamma_f  = FLAGS_gamma_f;
  ipm_parameter.gamma_a  = FLAGS_gamma_a;
  ipm_parameter.tau      = FLAGS_tau;
  ipm_parameter.algorithm= FLAGS_algorithm;
  // Begin timing
